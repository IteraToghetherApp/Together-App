...
export interface ValidateMemberAlertTokenParams {
...
export const validateMemberAlertToken = (params: ValidateMemberAlertTokenParams): void | never => {
    const {alertToken, member} = params;

    if (!alertToken || alertToken !== member.alertToken) {
        throw new InvalidCheckInTokenError();
    }
};
